<div x-data="{ open: @entangle('reminders_modal'), close: @entangle('reminders_modal') }">
    ...
    <x-card>
      ...
      <x-slot name="footer">
        <div class="flex justify-between gap-x-2">
          <x-button negative label="Cancel" wire:click="$set('deposit_modal', false)" />
  ...
  <x-modal wire:model.defer="amenities_modal" align="center">
    <x-card>
      ...
      <x-slot name="footer">
        <div class="flex justify-between gap-x-2">
          <x-button negative label="Cancel" wire:click="$set('amenities_modal', false)" />
          <div class="flex space-x-3">
  ...
  {{-- SUPERVISOR MODULE ON HOLD — restored authorization-cancel modal from master --}}
  <x-modal wire:model.defer="autorization_cancel_modal" align="center" max-width="md">
    <x-card>
      ...
      <div class="mt-7">
        <input type="password" wire:model="code"
          class="w-full text-lg @error('code') border-red-500 @enderror rounded-lg">
      </div>
      @error('code')
        <span class="text-sm text-red-500 mt-1">{{ $message }}</span>
      @enderror
